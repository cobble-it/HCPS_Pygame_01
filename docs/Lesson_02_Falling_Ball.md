...
<img src="../img/falling_ball.png" width="600"></img>
...
#### A library is a collection of pre-written code you can borrow. pygame gives us tools to make games, drawing shapes, handling the game window, and tracking time.
...
```
#### Constants are variables that never change while the program runs. We write them in ALL_CAPS as a signal to other programmers: "don't change this!" Colors in Pygame use RGB values — three numbers (0–255) for Red, Green, and Blue.
...
main()
```
#### A function is a reusable block of code with a name. def defines it, and you call it by writing main(). This keeps our code organized.
...
```
#### Every game runs a loop that repeats ~10 times per second. Each pass through the loop is one frame. Each frame we:
